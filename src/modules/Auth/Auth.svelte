...
<form method="POST" class="mx-auto grid gap-y-4" use:enhance={({cancel}) => {
	if (!validateForm()) {
		return cancel();
	}
	return async ({result}) => {
	...
	<div>
		<Input className="py-1 px-2" name="email" bind:value={email} type="email" placeholder="Email" />
	...
	<div>
		<Input className="py-1 px-2" name="password" type="password" placeholder="Password" bind:value={password} />
