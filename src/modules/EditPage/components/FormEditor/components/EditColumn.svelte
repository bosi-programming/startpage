...
<aside id={selectedColumn.title} class="form-editor__content">
  <div class="field">
    <label class="mr-4 dark:text-white" for={`title-${selectedColumn}`}
      >Title:</label
    >
    <input
      type="text"
      ...
      {#each selectedColumn.sites as site, siteIndex}
        <li class="m-0 rounded-none p-0 align-baseline">
          ...
          <button
            ...
            type="button"
            onclick={() => deleteSite(siteIndex, selectedColumnIndex)}
            >Delete</button
          >
        </li>
      {/each}
  ...
  <Button
    ...
    onclick={() => addNewSite(selectedColumnIndex)}>Add new site</Button
  >
</aside>
