...
export async function saveUserConfig(
  cookies: Cookies,
  configFromFE: Omit<Config, 'user' | 'id' | 'updatedAt'>,
  configId?: number,
): Promise<
...
> {
  ...
  if (configId) {
    ...
    if (currentConfig.userId !== user.id) {
      return [
        { errorCode: 400, errorMessage: 'User does not own config' },
        null,
      ];
    }
    const [configError, updatedConfig] = await configModel.put({
      ...configFromFE,
  ...
  const newConfig = configModel.repository.create({
    ...configFromFE,
    updatedAt: new Date().getTime(),
    userId: user.id,
    user,
  });
  const [userSaveError] = await userModel.put({
    ...user,
    config: newConfig,
